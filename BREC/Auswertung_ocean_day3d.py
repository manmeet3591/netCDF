...
def writeFile(year):
    location = ('/silos/boergel/BREC/files/auswertung' + year + '.nc')
...
def writeFilemean():
    location = ('/silos/boergel/BREC/files/mean/ocean_day3d_means.nc')
...
for i in range (1850, 2010):
    ...
    temp_field = numpy.mean(temp_vert, axis=1)
    temp_field = numpy.mean(temp_field, axis=1)
    salt_field = numpy.mean(salt_vert, axis=1)
    salt_field = numpy.mean(salt_field, axis=1)
